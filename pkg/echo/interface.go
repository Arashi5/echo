// Code generated by gitlab.lenvendo.ru/product/grade-factor/services/service-generator  REMOVE THIS STRING ON EDIT OR DO NOT EDIT.
//go:generate mockgen -destination service_mock.go -package echo  "github.com/arashi5/echo/pkg/echo Service
package echo

import (
	"context"

	_ "github.com/golang/mock/mockgen/model"
)

type Service interface {

	// CreateEcho returns buid time, last commit and version app
	CreateEcho(context.Context, *CreateEchoRequest) (*CreateEchoResponse, error)

	// GetEcho returns buid time, last commit and version app
	GetEcho(context.Context, *GetEchoListRequest) (*GetEchoListResponse, error)
}
